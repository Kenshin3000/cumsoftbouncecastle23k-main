...
bool match(TreeNode* tnode, ListNode* lnode){
    if (tnode->val == lnode->val) {
        if (lnode->next != nullptr) {
            if (tnode->left != nullptr && match(tnode->left, lnode->next))
                return true;
            else if (tnode->right != nullptr && match(tnode->right, lnode->next))
                return true;
            return false;
        }
        return true;
...
bool LinkedListInTree(TreeNode* binaryTreeHead, ListNode* linkedListHead)
{
    ...
    if (linkedListHead == nullptr) return true;  // null tree always exists at child nodes
    if (binaryTreeHead == nullptr) return false; // null tree cannot have any non-empty list

    bool res = false;

    // BFS
    queue<TreeNode*> qq;
    qq.push(binaryTreeHead);

    while (!qq.empty()) {
        auto tnode = qq.front();
        qq.pop();
        //cout << "matching " << tnode->val << endl;

        if (tnode->left  != nullptr) qq.push(tnode->left);
        if (tnode->right != nullptr) qq.push(tnode->right);

        if (match(tnode, linkedListHead)) {
            res = true;
            break;
        }
    }
