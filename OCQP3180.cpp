...
bool IsTippingPoint(const std::vector<int>& numbers, int index)
{
    ...
    std::vector<int> behind(numbers.size()), forward(numbers.size());
    partial_sum(numbers.begin(), numbers.end(), behind.begin());
    partial_sum(numbers.rbegin(), numbers.rend(), forward.rbegin());
    for(unsigned int i = 1; i < numbers.size() - 1; i++){
        if(behind[i-1] == forward[i+1]){
            if(i == index)
                return false;
